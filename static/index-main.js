var customFilter=(function(){function fLogger(){let div=document.createElement('div');let btn=document.createElement('button');btn.addEventListener('click',function(e){let q=$('thisone').value;console.log(q);});btn.textContent="LOG";div.appendChild(btn);document.body.appendChild(div);}
var selectedEntries;function searchFilter(){const startTime=performance.now();let selectedEntriesArray=new Array(...selectedEntries);if(isMobile){let selectedOptions=document.getElementById("customFilter-select").selectedOptions;for(let i=0;i<selectedOptions.length;i++){if(selectedOptions[i].value){selectedEntriesArray.push(selectedOptions[i].value);}}}
if(selectedEntriesArray.length<1){return;}
selectedEntriesArray.sort();let selectedEntriesObjectArray
...
return false;}
var isMobile=false;function fMain(entries){fx(entries);isMobile=determineMobile();$('.thisone').chosen({'width':'100%'});if(!isMobile){$('.thisone').on('change',changeHandler);}
...
else{return a[0]>b[0];}});let sel=document.createElement('select');let dummyOption=document.createElement('option');dummyOption.value="";sel.appendChild(dummyOption);countryArrMain=[];for(let i=0;i<countryArr.length;i++){let temp={};temp["c"]=countryArr[i][0];let arr=new Array(...countryArr[i][1]);arr=arr.sort();temp["l"]=arr;countryArrMain[i]=temp;}
for(let i=0;i<countryArrMain.length;i++){let optGrp=document.createElement('optgroup');optGrp.label=countryArrMain[i]["c"];let z=countryArrMain[i]["l"];let j=0;for(let j=0;j<z.length;j++){let opt=document.createElement('option');let k=JSON.parse(z[j]);opt.text=k.full||"Unknown";opt.value=""+i+"-"+j;optGrp.appendChild(opt);}
...
var locationData;var renderFunction;return{init:function(logging,loc_data,_renderFunction){renderFunction=_renderFunction;locationData=loc_data;fMain(loc_data);if(logging){fLogger();}}};})();